"""
Parse 「Second Stage 直し９. 2.docx」 (client review, 2026-08-19) into
...
Output: tool/corrections_0902.json
    {
      "source": "...docx",
      "word_count": N,
      "records": [
...
SRC = ROOT / 'Second Stage 直し９. 2.docx'
OUT = Path(__file__).with_name('corrections_0902.json')
...
def parse() -> dict:
    ...
    for raw in lines:
        ...
        if t.strip('<>　 ') in ('First Stage', 'Second Stage'):
            stage = t.strip('<>　 ')
            continue
...
def main() -> None:
    ...
    by_stage = Counter(r['stage'] for r in recs)
    for k, v in by_stage.items():
        print(f'  stage {str(k):13s}: {v} blocks')
